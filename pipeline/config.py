...
SEO_CATEGORIES = [
    ...
    (
        "rn",
        "Registered Nurse",
        r"\bRN\b|\bregistered\s+nurse\b",
        "Find registered nurse (RN) jobs. Browse {count} open RN positions updated daily.",
    ),
    (
        "lpn",
        "Licensed Practical Nurse",
        r"\bLPN\b|\blicensed\s+practical\s+nurse\b",
        "Browse {count} LPN jobs. Licensed practical nurse positions updated daily.",
    ),
    (
        "lvn",
        "Licensed Vocational Nurse",
        r"\bLVN\b|\blicensed\s+vocational\s+nurse\b",
        "Browse {count} LVN jobs. Licensed vocational nurse positions updated daily.",
    ),
    (
        "cna",
        "Certified Nursing Assistant",
        r"\bCNA\b|\bcertified\s+nurs(?:e|ing)\s+assistant\b|\bnurse\s+aide\b",
        "Browse {count} CNA jobs. Certified nursing assistant positions updated daily.",
    ),
    (
        "nurse-practitioner",
        "Nurse Practitioner",
        r"\bNP\b|\bnurse\s+practitioner\b|\bAPRN\b|\badvanced\s+practice\b",
        "Browse {count} nurse practitioner (NP/APRN) jobs updated daily.",
    ),
    (
        "case-manager",
        "Case Manager",
        r"\bcase\s+manag(?:er|ement)\b",
        "Browse {count} nurse case manager jobs updated daily.",
    ),
    # Specialty
    (
        "icu-nurse",
        "ICU Nurse",
        r"\bICU\b|\bintensive\s+care\b|\bcritical\s+care\b",
        "Browse {count} ICU nurse jobs. Intensive care nursing positions updated daily.",
    ),
    (
        "er-nurse",
        "ER Nurse",
        r"\bER\s+nurs|\bemergency\b.*nurs|\bnurs.*\bemergency\b|\bemergency\s+(?:room|department)\b",
        "Browse {count} ER nurse jobs. Emergency room nursing positions updated daily.",
    ),
    (
        "or-nurse",
        "OR Nurse",
        r"\bOR\s+nurs|\bsurg(?:ical|ery)\b.*nurs|\bnurs.*\bsurg(?:ical|ery)\b|\boperating\s+room\b",
        "Browse {count} OR and surgical nurse jobs updated daily.",
    ),
    (
        "nicu-nurse",
        "NICU Nurse",
        r"\bNICU\b|\bneonatal\b",
        "Browse {count} NICU nurse jobs. Neonatal ICU nursing positions updated daily.",
    ),
    (
        "pacu-nurse",
        "PACU Nurse",
        r"\bPACU\b",
        "Browse {count} PACU nurse jobs. Post-anesthesia care nursing positions.",
    ),
    (
        "med-surg",
        "Med-Surg Nurse",
        r"\bmed[\s\-]?surg\b|\bmedical[\s\-]?surgical\b",
        "Browse {count} med-surg nurse jobs. Medical-surgical nursing positions updated daily.",
    ),
    (
        "oncology-nurse",
        "Oncology Nurse",
        r"\boncology\b.*nurs|\bnurs.*\boncology\b|\bcancer\b.*nurs",
        "Browse {count} oncology nurse jobs. Cancer care nursing positions updated daily.",
    ),
    (
        "pediatric-nurse",
        "Pediatric Nurse",
        r"\bpediatric\b.*nurs|\bnurs.*\bpediatric\b|\bPICU\b",
        "Browse {count} pediatric nurse jobs updated daily.",
    ),
    (
        "psychiatric-nurse",
        "Psychiatric Nurse",
        r"\bpsych\b.*nurs|\bnurs.*\bpsych\b|\bmental\s+health\b|\bbehavioral\s+health\b",
        "Browse {count} psychiatric nurse jobs. Mental health nursing positions.",
    ),
    (
        "labor-delivery",
        "Labor & Delivery Nurse",
        r"\blabor\b.*\bdelivery\b|\bL&D\b|\bOB\b.*nurs|\bobstetric\b",
        "Browse {count} labor and delivery nurse jobs updated daily.",
    ),
    # Settings
    (
        "home-health",
        "Home Health Nurse",
        r"\bhome\s+health\b|\bhome\s+care\b|\bhospice\b",
        "Browse {count} home health nurse jobs. Home care nursing positions updated daily.",
    ),
    (
        "travel-nurse",
        "Travel Nurse",
        r"\btravel\s+nurs(?:e|ing)\b",
        "Browse {count} travel nurse jobs. Travel nursing positions across all states.",
    ),
    (
        "remote-nurse",
        "Remote Nursing",
        r"\bremote\b|\btelehealth\b|\btelemedicine\b|\bvirtual\s+(?:care|nurse|nursing)\b",
        "Browse {count} remote nursing jobs. Telehealth and work-from-home nursing positions.",
    ),
    # Leadership
    (
        "charge-nurse",
        "Charge Nurse",
        r"\bcharge\s+nurse\b",
        "Browse {count} charge nurse jobs updated daily.",
    ),
    (
        "nurse-manager",
        "Nurse Manager",
        r"\bnurse\s+manager\b|\bnursing\s+manager\b|\bdirector.*nurs|\bnurs.*director\b",
        "Browse {count} nurse manager jobs. Nursing management positions updated daily.",
    ),
    (
        "clinical-nurse",
        "Clinical Nurse",
        r"\bclinical\s+nurse\b",
        "Browse {count} clinical nurse jobs. Clinical nursing positions updated daily.",
    ),
    # Advanced practice
    (
        "crna",
        "Nurse Anesthetist",
        r"\bCRNA\b|\bnurse\s+anesthetist\b|\banesthesia\b.*nurs",
        "Browse {count} CRNA jobs. Certified registered nurse anesthetist positions.",
    ),
    (
        "midwife",
        "Midwife",
        r"\bmidwi(?:fe|very|ves)\b",
        "Browse {count} midwife jobs. Certified nurse-midwife positions updated daily.",
    ),
    (
        "nurse-educator",
        "Nurse Educator",
        r"\bnurse\s+educator\b|\bnursing\s+(?:instructor|faculty|professor|educat)\b|\bclinical\s+educator\b",
        "Browse {count} nurse educator jobs. Nursing education positions updated daily.",
    ),
    # Shift / employment type
    (
        "night-shift",
        "Night Shift Nurse",
        r"\bnight\s*shift\b|\bnoc(?:turnal)?\s+shift\b|\bovernight\b|\b3rd\s+shift\b|\bthird\s+shift\b",
        "Browse {count} night shift nursing jobs updated daily.",
    ),
    (
        "per-diem",
        "Per Diem Nurse",
        r"\bper\s*[\-\s]?diem\b|\bPRN\b",
        "Browse {count} per diem and PRN nursing jobs updated daily.",
    ),
    (
        "part-time-nurse",
        "Part-Time Nurse",
        r"\bpart[\s\-]?time\b",
        "Browse {count} part-time nursing jobs updated daily.",
    ),
    # Pay
    (
        "nursing-with-salary",
        "Jobs with Salary",
        None,
        "Browse {count} healthcare jobs with published salary ranges. Know your pay upfront.",
    ),
    # Allied Health — Therapy
    (
        "physical-therapist",
        "Physical Therapist",
        r"\bphysical\s+therap(?:ist|y)\b|\bPT\b(?=.*(?:therap|rehab|physical|clinic|hospital|health))|\bDPT\b|\bPTA\b",
        "Browse {count} physical therapist jobs. PT and PTA positions updated daily.",
    ),
    (
        "occupational-therapist",
        "Occupational Therapist",
        r"\boccupational\s+therap(?:ist|y)\b|\bOTR\b|\bCOTA\b",
        "Browse {count} occupational therapist jobs. OT and COTA positions updated daily.",
    ),
    (
        "speech-language-pathologist",
        "Speech-Language Pathologist",
        r"\bSLP\b|\bspeech[\s\-]?language\s+patholog(?:ist|y)\b|\bspeech\s+therap(?:ist|y)\b|\bCCC[\-\s]?SLP\b",
        "Browse {count} speech-language pathologist jobs. SLP positions updated daily.",
    ),
    (
        "respiratory-therapist",
        "Respiratory Therapist",
        r"\brespiratory\s+therap(?:ist|y)\b|\bRRT\b|\bCRT\b",
        "Browse {count} respiratory therapist jobs. RT positions updated daily.",
    ),
    # Allied Health — Diagnostic / Lab
    (
        "radiology-technologist",
        "Radiology Technologist",
        r"\brad(?:iologic(?:al)?|iology)\s+tech|\bRT\s*\(R\)|\bx[\-\s]?ray\s+tech|\bCT\s+tech|\bMRI\s+tech|\bsonograph|\bultrasound\s+tech|\bmammograph|\bdiagnostic\s+imaging",
        "Browse {count} radiology and imaging technologist jobs updated daily.",
    ),
    (
        "lab-technician",
        "Lab Technician",
        r"\bMLT\b|\bMLS\b|\bCLS\b|\bmedical\s+lab(?:oratory)?\s+(?:tech|scientist)|\bclinical\s+lab(?:oratory)?\s+scientist|\blab(?:oratory)?\s+tech(?:nician|nologist)?",
        "Browse {count} medical laboratory technician and scientist jobs updated daily.",
    ),
    (
        "phlebotomist",
        "Phlebotomist",
        r"\bphlebotom(?:ist|y)\b",
        "Browse {count} phlebotomist jobs updated daily.",
    ),
    # Allied Health — Pharmacy / Nutrition
    (
        "pharmacist",
        "Pharmacist",
        r"\bPharmD\b|\bRPh\b|\bpharmac(?:ist|y\s+tech)|\bpharmacy\s+tech(?:nician)?",
        "Browse {count} pharmacist and pharmacy technician jobs updated daily.",
    ),
    (
        "dietitian",
        "Dietitian",
        r"\bRDN?\b(?=.*(?:diet|nutri|food|clinical|health))|\bdietit(?:ian|ion)\b|\bnutritionist\b|\bclinical\s+nutrition\b",
        "Browse {count} dietitian and nutritionist jobs updated daily.",
    ),
    # Allied Health — Other
    (
        "medical-assistant",
        "Medical Assistant",
        r"\bmedical\s+assistant\b|\bCMA\b|\bclinical\s+medical\s+assistant\b",
        "Browse {count} medical assistant jobs updated daily.",
    ),
    (
        "surgical-technologist",
        "Surgical Technologist",
        r"\bsurg(?:ical)?\s+tech(?:nolog(?:ist|y))?\b|\bCST\b|\boperating\s+room\s+tech\b",
        "Browse {count} surgical technologist jobs updated daily.",
    ),
    (
        "paramedic",
        "Paramedic / EMT",
        r"\bparamedic\b|\bEMT\b|\bemergency\s+medical\s+tech(?:nician)?",
        "Browse {count} paramedic and EMT jobs updated daily.",
    ),
    (
        "dental-hygienist",
        "Dental Hygienist",
        r"\bdental\s+hygien(?:ist|e)\b|\bRDH\b",
        "Browse {count} dental hygienist jobs updated daily.",
    ),
    (
        "social-worker",
        "Healthcare Social Worker",
        r"\bLCSW\b|\bLMSW\b|\bmedical\s+social\s+worker\b|\bclinical\s+social\s+worker\b|\bhealthcare\s+social\s+worker\b",
        "Browse {count} healthcare social worker jobs updated daily.",
    ),
    (
        "athletic-trainer",
        "Athletic Trainer",
        r"\bathletic\s+train(?:er|ing)\b|\bATC\b",
        "Browse {count} athletic trainer jobs updated daily.",
    ),
]
...
_SUFFIX_SPLIT_RE = re.compile(r"(" + "|".join(_SLUG_SUFFIXES) + r")$", re.IGNORECASE)
